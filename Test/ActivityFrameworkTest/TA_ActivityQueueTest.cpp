...
TEST_F(TA_ActivityQueueTest, getFront)
{
    ...
    auto activity = CoreAsync::ITA_ActivityCreator::create(&MetaTest::sub, m_pTest, 6,3);
...
TEST_F(TA_ActivityQueueTest, multiThreadTest)
{
    ...
    std::function<bool()> func_1 = [&]() {
        for(int i = 0;i < 150;++i)
        {
            queue.push(std::make_shared<CoreAsync::TA_ActivityProxy>(CoreAsync::ITA_ActivityCreator::create(&MetaTest::sub, m_pTest, i,3)));
        }
        return true;
    };
